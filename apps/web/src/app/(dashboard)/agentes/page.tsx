...
interface AgentUser { id: string; firstName: string; lastName: string; email: string; createdAt: string; }

export default function AgentesPage() {
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['admin-agentes'],
    queryFn: () => api.get('/api/users?role=AGENTE').then(r => r.data.data as AgentUser[]),
  });
  const users = data ?? [];

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold text-gray-900" style={{ fontFamily: 'var(--font-display)' }}>Agentes Aduanales</h1>
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        ...
        : users.length === 0 ? <div className="p-10 text-center"><UserCheck className="w-8 h-8 text-gray-200 mx-auto mb-2" /><p className="text-gray-400 text-sm">Sin agentes registrados</p></div>
