...
// POST /api/automation/credentials — Guardar credenciales cifradas
...
router.post('/sigie/:id',
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({ success: true, jobId: job?.id, message: 'Solicitud SIGIE encolada. Recibirás una notificación cuando sea procesada.' });
  })
);
...
// POST /api/automation/sat/:id — Transmitir DUCA-D (BLOQUEA si SIGIE no aprobado)
router.post('/sat/:id',
  requireRole('AGENTE', 'ADMIN', 'SUPERADMIN'),
  requireSIGIEApproved('id'),
  asyncHandler(async (req, res) => {
...
// GET /api/automation/jobs/:jobId — Ver estado de un job
